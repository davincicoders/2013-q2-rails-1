...
post "/login" do
  # TODO find a user with the username that the user posted from the form
  @user = CarShareMember.where(username: params["username"]).first

  # TODO check if that user exists and their password is correct.
  if @user != nil && @user.authenticate(params["password"])
    # TODO if so, save the user's ID to session and redirect to /reservations
    session[:car_share_member_id] = @user.id
    redirect "/reservations"
  else
    # TODO if not, redirect to login page again with a flash message:
...
get "/reservations" do
  # TODO check if there's a user ID saved in the session
  if session[:car_share_member_id] != nil
    # TODO If so, load a variable named @user, and show the reservations ERb.
    ...
    # TODO If not, redirect to the login page.
    redirect "/login"
  end
end
